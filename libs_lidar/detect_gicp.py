...
class FastLidarMapper:
    def __init__(self, load_from_path=None): # Thêm tham số để tải từ đường dẫn
        ...
        print(f"center_px = {self.center_px}")
    ...
    def process_scan(self, scan_pts0, integrate=True, distan_scan_icp = 20000, distan_scan_update = 10000, 
                     x_agv_pixel = 0, y_agv_pixel = 0, goc_agv_do = 0, cap_nhat_vi_tri = 0, cap_nhat_ban_do = 0,
                     update_all_point_in_map = False, scan_an_toan = []):
        ...
        scan_an_toan_pixel = []
        if len(scan_an_toan) != 0:
            scan_an_toan = (np.array(scan_an_toan) @ R.T) + tvec
            # [[-273.4461767900675, -750.9417793504556, 0.14717896349246873]]
            px_an_toan = scan_an_toan[0,0]
            py_an_toan = scan_an_toan[0,1]
            px_an_toan_pixel = int(self.center_px[0] + px_an_toan / self.resolution_mm)
            py_an_toan_pixel = int(self.center_px[1] - py_an_toan / self.resolution_mm)
            scan_an_toan_pixel = [[px_an_toan_pixel, py_an_toan_pixel]]
            # print(scan_an_toan_pixel)
            

        return self.center_px, pts_world, rmse, self.tam_x_mm, self.tam_y_mm, self.tam_x_pixel, self.tam_y_pixel, self.goc_agv, self.resolution_mm, scan_an_toan_pixel
